...
function Voiceplayer() {
  ...
  return (
    <>
      ...
      <img alt="" className="dataimg" src={pictures[currentindex]} />
      <br></br>
      <audio
        className="myaudio"
        controls
        src={myAudio[currentindex]}
        ref={myRef}
        type={"audio/mp3"}
      />
      <br></br>
      <div className="buttons">
        ...
        <button onClick={() => backward()} className="prev">
          <GiPreviousButton />
        </button>
        <button onClick={() => setplaying(!playing)} className="play">
          {playing ? <BsPauseCircle /> : <BsPlayCircle />}
        </button>
        <button onClick={() => forward()} className="next">
          {" "}
          <GiNextButton />
        </button>
